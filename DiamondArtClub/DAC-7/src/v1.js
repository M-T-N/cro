...
*/
  ...
  qs('head').insertAdjacentHTML(
    ...
    <style>
      ...
      .tin_section header p {
        padding: 20px 10px;
        margin: 0;
      }
      .tin_section header .OffcanvasNav-chevron {
        right: 20px;
        border-top: 0.2rem solid #2d2d2d;
        border-right: 0.2rem solid #2d2d2d;
        transform: translateY(-65%) translateZ(0) rotate(45deg);
        width: 15px;
        height: 15px;
      }
      .tin_section__reviews, .tin_section__everything-needed {
        border-bottom: 1px solid black;
      }
      .tin_section__content {
        padding: 0 15px 10px;
        transition: all 200ms ease-in;
        height: 100%;
      }
      .tin_section.collapsed header .OffcanvasNav-chevron {
        transform: translateY(-65%) translateZ(0) rotate(135deg);
      }
      .tin_section.collapsed .tin_section__content {
        height: 0;
        opacity: 0;
        padding: 0;
        pointer-events: none;
      ...
      .tin_section .product_small_title {
        font-family: Brown-Bold;
        letter-spacing: 0;
        text-transform: uppercase;
        font-size: 14px;
      }
      .tin_section .how_to_use_block {
        text-align: center;
      }
      .tin_section .how_to_use_block .title_text {
        text-transform: uppercase;
        font-weight: 600;
        margin-bottom: 10px;
        font-size: 18px;
      ...
      .tin_section .how_to_use_block img {
        width: 160px;
        height: 160px;
      }
      .tin_section__content .how_to_use_sec, .tin_section__content .diamond_difference_sec  {
        padding-top; 0;
      }
      .okeReviews-reviewsWidget-reviewsAggregate, .ShapeCurve, .Product-artist {
        display: none;
      }
    </style>
  `
  ...
  */
  ...
  // 10 REVIEW SECTION
  const reviewTitle = qs(
    '#customer-reviews-container h2:not(.okeReviews-review-main-heading)'
  ).outerHTML;
  const writeReview = qs(
    '#customer-reviews-container .okeReviews-reviewsWidget-header-controls'
  ).outerHTML;
  const reviewTotal = qs('.okeReviews-reviewsAggregate-summary').outerHTML;
  const firstReview = qs('.okeReviews-reviews-review:first-of-type').outerHTML;
  const allReviews = qs('.okeReviews-reviews-main').outerHTML;
  const loadMoreReviews = qs('.okeReviews-reviews-footer').outerHTML;
  /*
  qsa(
    '.okeReviews-reviews-review:not(.okeReviews-reviews-review:first-of-type)'
  ).forEach(function (el) {
    otherReviews = otherReviews.concat([...el.outerHTML], '');
  });
  */
  // const reviews = qs('#customer-reviews-container').outerHTML;

  // remove old previous sections
  qsa(
    '.Product-content .PDPmobileform, .product_desc_block, .ProductDropdowns, .product_desc_block, .how_to_use_steps_wrapper, .everthing_you_include_sec, .collection_shop_by_category, .custreview_client_logo_wrapper, #shopify-section-lpg-diamond-difference, .usefull_links_wrapper, #customer-reviews-container'
  ).forEach(function (el) {
    el.remove();
  });
  ...
  qs('.buy-now--row').insertAdjacentHTML(
    ...
    <section class="tin_section tin_section__reviews">
      <header></header>
      <div class="tin_section__content">
        ${reviewTitle}
        <div data-oke-reviews-version="2.13.1" class="okeReviews okeReviews--theme js-okeReviews">
          ...
          <div class="okeReviews-reviewsWidget-reviews js-okeReviews-reviewsContainer">
            <div class="okeReviews-reviews">
              ${firstReview}
              <div class="okeReviews-reviews-footer js-okeReviews-footer js-okeReviews-reviews-footer">
                <button class="okeReviews-reviews-showMore okeReviews-reviews-showMore--tin okeReviews-showMore js-okeReviews-showMore">Show more</button>
              <div class="tin__more-reviews tin__more-reviews--hidden">
                ${allReviews}
                </div>
              </div>
            </div>
  ...
  // show more reviews
  qs('.okeReviews-reviews-showMore--tin').addEventListener(
    'click',
    function () {
      qs('.tin__more-reviews').classList.remove('tin__more-reviews--hidden');
    }
  );
}
