...
function init() {
  ...
  console.log('HGH-21 V1 | (Horizon) Ecomm Pop Up Iteration');
    ...
    function nextCard(){
      ...
      qsa('.tin__booking-card__close').forEach(el => {
        ...
        el.addEventListener('click', () => {
          ...
          //window.clearTimeout(timeoutID);
          ga('send', 'event', 'cro conversion', 'HGH21_V1: Popup close button');
          ga('send', 'event', 'cro conversion', 'HGH21_V1: Popup close button'  + cardID);
        })
      });
      ...
      qs('.left-area .cta-ezbook a').addEventListener('click', () => {
        ga('send', 'event', 'cro conversion', "HGH21_V1: EZBook Navbar click");
      });

      /*
      qsa('.tin__booking-card__content__header__link a').forEach(el => {
        el.addEventListener('click', () => {
          ga('send', 'event', 'cro conversion', "HGH21_V1: What's Included clicks");
        });
      });
      */
      ...
      qsa('.tin__booking-card__content__footer__cta a').forEach(el => {
        var cardID = el.closest('.tin__booking-card').id;
        el.addEventListener('click', () => {
          ga('send', 'event', 'cro conversion', 'HGH21_V1: EZBook Online popup click');
          ga('send', 'event', 'cro conversion', 'HGH21_V1: EZBook Online popup click' + cardID);
        });
      });
