...
function init() {
  ...
  console.log('HGH22_v1 - Regular Appointments');
  ...
  setTimeout(function(){
    ...
    qsa('.tin__booking-card__content__footer__cta a').forEach(el => {
      el.addEventListener('click', () => {
        ga('send', 'event', 'cro conversion', `HGH22_v1-RegularAppointments: EZBook Online popup click - ${el.closest('.tin__booking-card').id}`);
      });
    });
