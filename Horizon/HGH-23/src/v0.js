...
function init() {
  ...
  setTimeout(function(){
    ...
    qsa('.tin__booking-card__content__header__link a').forEach(el => {
      el.addEventListener('click', () => {
        ga('send', 'event', 'cro conversion', `HGH23: What's Included popup click - ${el.closest('.tin__booking-card').id}`);
      });
    });
    ...
    qsa('.tin__booking-card__content__footer__cta a').forEach(el => {
      el.addEventListener('click', () => {
        ga('send', 'event', 'cro conversion', `HGH23: EZBook Online popup click - ${el.closest('.tin__booking-card').id}`);
      });
    });
