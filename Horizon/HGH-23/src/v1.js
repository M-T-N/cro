...
import { card1, card2, card3, card4, card5} from './utils/cardContent2';
    ...
    function nextCard(){
      ...
      qsa('.tin__booking-card__close').forEach(el => {
        ...
        el.addEventListener('click', () => {
          ...
          //window.clearTimeout(timeoutID);
          ga('send', 'event', 'cro conversion', 'HGH23_V1: Popup close button');
          ga('send', 'event', 'cro conversion', 'HGH23_V1: Popup close button ' + cardID);
        })
      });
      ...
      qs('.left-area .cta-ezbook a').addEventListener('click', () => {
        ga('send', 'event', 'cro conversion', "HGH23_V1: EZBook Navbar click");
      });
      ...
      */
      ...
      qsa('.tin__booking-card__content__footer__cta a').forEach(el => {
        var cardID = el.closest('.tin__booking-card').id;
        el.addEventListener('click', () => {
          ga('send', 'event', 'cro conversion', 'HGH23_V1: EZBook Online popup clicks');
          ga('send', 'event', 'cro conversion', 'HGH23_V1: EZBook Online popup clicks ' + cardID);
        });
      });
